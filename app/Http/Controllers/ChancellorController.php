    ...
    public function pendingDashboard(){
        ...
        if ($search) {
            $proposals = Research::where('title', 'LIKE', '%' . ($search) . '%')
                                    ...
                                    ->where(['location_from' => 'Dean'])
                                     ->where(['location_to' => 'Chancellor'])
        ...
        return view('chancellor.pending_dashboard', [
            'user' => $user,
            'proposals' => $proposals,
            'active' => "chancellorpending",
            'search' => $search
        ]);
       }
       ...
       public function revisionDashboard(){
        ...
        if ($search) {
            $proposals = Research::where('title', 'LIKE', '%' . ($search) . '%')
                                    ...
                                    ->whereNotIn('location_from',['Faculty', 'Program Chair','Dean'])   
                                    ->whereNotIn('location_to',['Faculty','Dean', 'Program Chair', 'RMO'])                           
                                    ->orderBy('created_at', 'desc')
                                    ->paginate(5);
        }
        else {
        $proposals = Research::whereNotIn('status',['PENDING', 'APPROVED'])   
                                ->whereNotIn('location_from',['Faculty', 'Program Chair','Dean'])   
                                ->whereNotIn('location_to',['Dean', 'Program Chair', 'RMO'])  
                             ->orderBy('created_at', 'desc')       
                             ->paginate(5);
        }
        return view('chancellor.revision_dashboard', [
            'user' => $user,
            'proposals' => $proposals,
            'active' => "chancellorrevision",
            'search' => $search
        ]);
       }
       ...
       public function rbpDashboard(){
        ...
        return view('chancellor.rbp_dashboard', [
            'user' => $user,
            'proposals' => $proposals,
            'search' => $search,
            'active' => "chancellorrbp",
        ]);
    }
    ...
    public function viewResearch($id)
        {
            ...
            return view('chancellor.view_rbp',[
                'proposal' => $proposal,
                'user' => $user,
                'members' => $members,
        ...
        public function validateResearch($id)
        {
            ...
            return view('chancellor.validate_rbp',[
                'proposal' => $proposal,
                'user' => $user,
                'members' => $members,
                'paperpublisheds' => $paperpublisheds,
        ...
        public function approveResearchChancellor($id)
        {
            $research = Research::where('id',$id)->first();
         
            $research->update([
                'location_from' => 'Chancellor',
                'location_to' => 'RMO',
                'station' => '4',
            ]);
            return redirect()->route('chancellor-pending',$id)->with('success', 'Research has been submitted.');
        }

        public function rejectResearchChancellor(Request $request,$id)
        {
            ...
            return redirect()->route('chancellor-pending',$id)->with('alert', 'Research has been rejected.');
        }
        ...
        public function viewPending($id)
        {
            ...
            return view('chancellor.view_pending',[
                'proposal' => $proposal,
                'user' => $user,
                'members' => $members,
                'paperpublisheds' => $paperpublisheds,
        ...
        public function viewPaperPublished($id)
        {
            ...
            return view('chancellor.view_paperpublished',[
                'user' => $user,
                'active' => "chancellorpending",
                'paperpublisheds' => $paperpublisheds,
            ]);
        ...
        public function viewPaperPresented($id)
        {
            ...
            return view('chancellor.view_paperpresented',[
                'user' => $user,
                'active' => "chancellorpending",
                'paperpresenteds' => $paperpresenteds,
            ]);

          ...
          <div class="row ml-2 mb-2 py-3">
            <div class="col-xl-6 col-lg-12 pl-0">
              <h3 class="card-heading py-3">Add Proposal Members:</h3>
                <form action="{{ route('rmo-invention-addmembers', $proposal->id) }}" method="GET" class="search-form">
                  @csrf
                      ...
                      @else
                        @if($faculties->count())
                          ...
                          <tr>
                            ...
                            <form action="{{ route('rmo-addmember',[$proposal->id, $item->id]) }}" method="POST">
                              @csrf
                              <input type="hidden" name="employee_no" value="{{ $item->employee_no }}">
                              <input type="hidden" name="fullname" value="{{ $item->fullname }}">
                              <input type="hidden" name="email" value="{{ $item->email }}">
                  ...
                  <table class="table table-rounded" >
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <form action="{{ route('rmo-invention-deletemember',[$proposal->id, $item->user_id]) }}" method="POST">
                          @csrf
                          @method('DELETE')
...
<a href="{{ route('rmo-invention-view', $proposal->id) }}" class="btn btn-primary float-right submit-proposal-btn"><i class="fas fa-check-circle"></i> DONE</a>
</div>
</div>
</div>

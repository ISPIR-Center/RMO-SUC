    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body text-center">
          ...
          <form action="{{ route('rmo-paperpresented-reject', $paperpresented->id) }}" method="post">
            @csrf
            @method('PATCH')
            <input type="hidden" name="status" value="Rejected by the RMO">
        ...
        <form action="{{ route('rmo-paperpresented-approve', $paperpresented->id) }}" method="POST">
          @csrf
          @method('PUT')
          <input type="hidden" name="status" value="Approved by the RMO">
          <button type="submit" class="btn btn-secondary float-lg-right"><i class="fas fa-check"></i></button>
        </form>
        <a href="/rmo/paper-presented/view/{{$paperpresented->id}}/{{ $proposal->id }}" class="btn btn-secondary float-lg-right"><i class="fas fa-eye"></i></a>
           </td>
         </tr>
          <!-- DELETE UTILIZED -->
 ...
 <div class="modal fade" id="publishedreviseModal-{{ $paperpublished->id }}" tabindex="-1" role="dialog" aria-labelledby="publishedreviseModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body text-center">
        <p>Are you sure you want to <span class="colored">REJECT</span> this information?</p> 
        <form action="{{ route('rmo-paperpublished-reject', $paperpublished->id) }}" method="post">
          @csrf
          @method('PATCH')
          <input type="hidden" name="status" value="Rejected by the RMO">
          <button type="submit" class="btn btn-primary ">Yes</button>
      ...
      <form action="{{ route('rmo-paperpublished-approve', $paperpublished->id) }}" method="POST">
        @csrf
        @method('PUT')
        <input type="hidden" name="status" value="Approved by the RMO">
        <button type="submit" class="btn btn-secondary float-lg-right"><i class="fas fa-check"></i></button>
      </form>
      <a href="/dean/paper-published/view/{{$paperpublished->id}}/{{ $proposal->id }}" class="btn btn-secondary float-lg-right"><i class="fas fa-eye"></i></a>
         </td>
       </tr>
      <!-- DELETE UTILIZED -->
 ...
 <div class="modal fade" id="journalcitedreviseModal-{{ $journalcited->id }}" tabindex="-1" role="dialog" aria-labelledby="journalcitedreviseModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body text-center">
        <p>Are you sure you want to <span class="colored">REJECT</span> this information?</p> 
        <form action="{{ route('rmo-journalcited-reject', $journalcited->id) }}" method="post">
          @csrf
          @method('PATCH')
          <input type="hidden" name="status" value="Rejected by the RMO">
          <button type="submit" class="btn btn-primary">Yes</button>
      ...
      <form action="{{ route('rmo-journalcited-approve', $journalcited->id) }}" method="POST">
        @csrf
        @method('PUT')
        <input type="hidden" name="status" value="Approved by the RMO">
        <button type="submit" class="btn btn-secondary float-lg-right"><i class="fas fa-check"></i></button>
 ...
 <div class="modal fade" id="bookcitedreviseModal-{{ $bookcited->id }}" tabindex="-1" role="dialog" aria-labelledby="bookcitedreviseModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body text-center">
        <p>Are you sure you want to <span class="colored">REJECT</span> this information?</p> 
        <form action="{{ route('rmo-bookcited-reject', $bookcited->id) }}" method="post">
          @csrf
          @method('PATCH')
          <input type="hidden" name="status" value="Rejected by the RMO">
          <button type="submit" class="btn btn-primary">Yes</button>
      ...
      <form action="{{ route('rmo-bookcited-approve', $bookcited->id) }}" method="POST">
        @csrf
        @method('PUT')
        <input type="hidden" name="status" value="Approved by the RMO">
        <button type="submit" class="btn btn-secondary float-lg-right"><i class="fas fa-check"></i></button>
  ...
  <tbody>
   ...
   @foreach ($patenteds as $patented)
    <tr class="unread text-center">
      ...
      <td>
        <a href="/rmo/rbp/view/{{$proposal->id}}" class="btn btn-secondary"><i class="fas fa-eye"></i></a>
        <a href="/rmo/rbp/view/{{$proposal->id}}" class="btn btn-secondary"><i class="fas fa-check"></i></a>
        <a href="/rmo/rbp/view/{{$proposal->id}}" class="btn btn-secondary"><i class="fas fa-times"></i></a>
      </td>
    </tr>
    <!-- Patented Modal Edit -->
...
<div class="modal fade" id="utilizedEditModal-{{ $utilized->id }}" tabindex="-1" role="dialog" aria-labelledby="utilizedEditModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
   ...
   @endif
    ...
    @else
     @endif
     <form action="{{ route('rmo-rbp-reject',$proposal->id) }}" method="post">
      @csrf
      @method('PUT')
     <div class="col-xl-12 ml-0">
     <div class="form-group text-left">
      <label for="InputUniversityAgenda" class="card-heading py-2">Remarks</label>
      <textarea class="form-control" id="remarks" onkeyup="putremarks()" name="remarks"></textarea>
    </div>
  </div>
  ...
  <form action="{{ route('rmo-rbp-approve',$proposal->id) }}" method="post">
    @csrf
    @method('PUT')
    @if($paperpresenteds->count() || $paperpublisheds->count() || $bookciteds->count() || $journalciteds->count() || $utilizeds->count() || $patenteds->count())
